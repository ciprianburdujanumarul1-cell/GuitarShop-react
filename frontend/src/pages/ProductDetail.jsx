...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const socket = new WebSocket(`${WS_URL}/ws/socket-server/${id}/`)
    ...
    socket.onmessage = (e) => {
      ...
      if (data.type === 'chat') {
        setReviews((prev) => [
          { id: `live-${Date.now()}`, username: data.username, message: data.message, rating: data.rating, created_at: new Date().toISOString() },
          ...prev,
        ])
      }
    }
    ...
    <>
      <div className="pd-container">
        <div className="pd-image">
          <img src={product.image || 'https://via.placeholder.com/450x400'} alt={product.name} />
        </div>
        ...
        <div className="pd-info">
          ...
          {product.avg_rating && (
            <div className="avg-rating">
              <span className="stars">{starsHtml(Math.round(product.avg_rating))}</span>
          ...
          <div className="pd-description">
            Body | {product.body}<br />
            Neck | {product.neck}<br />
            Grip Shape | {product.grip_shape}<br />
            Fingerboard | {product.fingerboard}<br />
            Fret | {product.fret}<br />
            Inlay | {product.inlay}<br />
            Scale | {product.scale}<br />
            Nut | {product.nut}<br />
            Construction | {product.construction}<br />
            Tuner | {product.tuner}<br />
            Bridge | {product.bridge}<br />
            Pickup | {product.pickup}<br />
            Controls | {product.controls}<br />
            Color | {product.color}
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
            ...
            <Link to="/wishlist" style={{ fontSize: 13, color: 'var(--muted)' }}>My Favorites</Link>
          </div>
        </div>

        {/* REVIEWS */}
        <div className="reviews-section">
          <h2>Re<span>views</span></h2>
            ...
            {reviews.map((review) => (
              <div className="review-card" key={review.id}>
                <div className="review-meta">
                  {review.username} · {new Date(review.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </div>
